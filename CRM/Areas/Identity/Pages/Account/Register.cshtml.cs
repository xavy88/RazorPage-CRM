    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!await _roleManager.RoleExistsAsync(SD.RootRole))
                {
                    _roleManager.CreateAsync(new IdentityRole(SD.RootRole)).GetAwaiter().GetResult();
                    ...
                    _roleManager.CreateAsync(new IdentityRole(SD.SalesRole)).GetAwaiter().GetResult();
                }
                if (result.Succeeded)
                {
                    if (role == SD.SalesRole)
                    {
                        await _userManager.AddToRoleAsync(user, SD.SalesRole);
                    }
                    else 
                    {
                        if (role == SD.RootRole) 
                        { 
                            await _userManager.AddToRoleAsync(user, SD.RootRole);
                        }
                        else
                        {
                            if (role == SD.SupervisorRole)
                            {
